...
const Notes = () => {
  ...
  const handleSummarizeSavedNote = async (id) => {
    ...
    try {
      ...
      fetchNotes(); // to update the note list with the new summary internally if needed
    } catch(err) {
      console.error(err);
    }
        ...
        <div className="flex-col gap-4">
          <div className="card">
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
               ...
               />
               <button className="btn-primary" onClick={handleAskQuestion} disabled={loading}>Ask</button>
            </div>
            {answer && (
              <div style={{ marginTop: '1rem', padding: '1rem', background: '#fef2f2', borderRadius: 'var(--radius)', border: '1px solid #fecaca', color: '#991b1b' }}>
              ...
              <div className="flex-col gap-2">
                {notes.map(n => (
                  <div key={n.id} style={{ padding: '0.75rem', border: '1px solid var(--border-color)', borderRadius: 'var(--radius)' }}>
                    ...
                    <div className="text-muted" style={{ fontSize: '0.85rem', marginTop: '0.5rem' }}>{n.content.substring(0, 50)}...</div>
